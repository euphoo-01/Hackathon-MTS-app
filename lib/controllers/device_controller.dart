...
class DeviceController extends ChangeNotifier {
  ...
  // Загрузка устройств из локального хранилища (для демонстрации)
  Future<void> _loadDevicesFromStorage() async {
    try {
      ...
      if (devicesJson != null) {
        ...
        _devices = decoded.map((item) => DeviceModel.fromJson(item)).toList();
      }
    } catch (e) {
      _error = e.toString();
    }
  ...
  Future<bool> addDevice(String name, String deviceCode) async {
    ...
    try {
      // Проверка, не существует ли уже устройство с таким кодом
      final existingDevice = _devices.any((d) => d.deviceCode == deviceCode);
      if (existingDevice) {
  ...
  Future<bool> addDevice3(String userId, String deviceCode, String deviceName) async {
